...
fun getOfflineModelConfig(type: Int): OfflineModelConfig? {
    when (type) {
        ...
        45 -> {
            val modelDir = "sherpa-onnx-whisper-small.en"
            return OfflineModelConfig(
                whisper = OfflineWhisperModelConfig(
                    encoder = "$modelDir/small.en-encoder.onnx",
                    decoder = "$modelDir/small.en-decoder.onnx",
                ),
                tokens = "$modelDir/small.en-tokens.txt",
                modelType = "whisper",
            )
        }
        ...
        48 -> {
            val modelDir = "sherpa-onnx-zipformer-gigaspeech-2023-12-12"
            return OfflineModelConfig(
                transducer = OfflineTransducerModelConfig(
                    encoder = "$modelDir/encoder-epoch-30-avg-1.int8.onnx",
                    decoder = "$modelDir/decoder-epoch-30-avg-1.int8.onnx",
                    joiner = "$modelDir/joiner-epoch-30-avg-1.int8.onnx",
                ),
                tokens = "$modelDir/tokens.txt",
                modelType = "transducer",
            )
